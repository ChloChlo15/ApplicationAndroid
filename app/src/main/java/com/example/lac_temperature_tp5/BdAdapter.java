...
public class BdAdapter {
    ...
    public Cursor getReleveWithDate(int jour, int mois){
        ...
        //return db.rawQuery("SELECT * FROM TABLE_RELEVE WHERE COL_JOUR = 21 AND COL_MOIS = 10", null);
    ...
    public Cursor TempByMois(int mois){
        //Récupère dans un Cursor les valeurs correspondant à un releve grave a sa date)
        return db.rawQuery("SELECT TEMPERATURE FROM TABLE_RELEVE WHERE MOIS = ? ", new String[] {String.valueOf(mois)});
    }
    public Cursor TempByJours(int jour, int mois){
        // Récupère dans cursor les valeurs correspondant à une releve grace au jour saisie par l'utilisateur
        return db.rawQuery("SELECT * FROM TABLE_RELEVE WHERE JOUR = ?", new String[] { String.valueOf(jour)});
    }
